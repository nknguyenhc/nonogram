...
public class ConstraintTest {
    ...
    public void testMultiplePossibility() {
        ...
        constraint.undo();
        constraint.update(new int[] {2, 1, 2, 1, 2});
        Assert.assertEquals(0, constraint.possibilityCount());
        constraint.undo();
        constraint.update(new int[] {1, 2, 2, 0, 0});
        Assert.assertEquals(0, constraint.possibilityCount());
        constraint.undo();
        constraint.update(new int[] {2, 0, 0, 2, 1});
        Assert.assertEquals(1, constraint.possibilityCount());
    }

    @Test
    public void testUndo() {
        Constraint constraint = new Constraint(new int[] {1, 1}, new int[] {2, 1, 0, 0, 0});
        Assert.assertEquals(3, constraint.possibilityCount());
